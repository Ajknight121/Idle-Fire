...
export default function UpgradeContainer(props: IGlobalAppProps) {
  const getUpgradeContent = (
    ...
    const classForUpgrade = (upgrade.unlocked && GameUpgradesFactory.canAfford(upgrade, props.appState))
        ? "upgrade-available"
        : "upgrade-unavailable";
    return (
      <Upgrade
  ...
  const buyUpgrade = (embers: number, cost: number, value: number): boolean => {
  ...
  const upgradeQuantity = (
    ...
    props.setAppState(
      GlobalAppState.updateBuyQuantity(buyQuantity, props.appState)
    );
  };
  ...
  const handleUpgradeCLick = (
    ...
    for (let i = 0; i < buyQuantity; i++) {
      if (buyUpgrade(embers, upgrade.upgradeCost, 1)) {
        props.setAppState(
          GlobalAppState.increaseUpgradeLvl(upgrade, props.appState)
        );
      }
    }
    ...
    <div className="upgrade-container">
      ...
      <div className={"upgrade-quantity"}>
        ...
        <input
          ...
          checked={props.appState.buyQuantity === 1}
        />{" "}
        x1
        <input
          type={"radio"}
          ...
          checked={props.appState.buyQuantity === 10}
        />{" "}
        x10
        <input
          type={"radio"}
          ...
          checked={props.appState.buyQuantity === 100}
        />{" "}
        x100
      </div>
      <div className={"upgrades"}>
        {props.appState.upgrades.map(getUpgradeContent)}

        {/*TODO: An onclick event could be added to the upgrades div that identifies which upgrade was clicked*/}
        {/*<div onClick={() => upgradeAction(props.appState.embers, 10, 1)}>*/}
        {/*  <Upgrade*/}
        {/*    key={"Grass plucker"}*/}
        {/*    unlocked={true}*/}
        {/*    classname={*/}
        {/*      props.appState.embers >= 10*/}
        {/*        ? "upgrade-available"*/}
        {/*        : "upgrade-unavailable"*/}
        {/*    }*/}
