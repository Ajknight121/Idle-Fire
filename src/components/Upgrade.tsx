        ...
        <HtmlTooltip title={
          <React.Fragment>
            ...
            {props.upgradeProps.quantity > 0 ? <div className={"tooltip-values"}>
              <div>{`Each ${props.upgradeProps.upgradeName} produces ${props.upgradeProps.EPS} embers per second`}</div>
              <div>{`${props.upgradeProps.upgradeName} produces a total of ${props.upgradeProps.EPS * appState.buyQuantity} embers per second`}</div>
            </div> : ""}
